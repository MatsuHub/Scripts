...
local function startFly(s)
    ...
    task.spawn(function()
        while flying and h.Parent do
            ...
            -- Lógica 1: Passou da cachoeira e ainda não clicou em descer
            if posZ > 9410 and posZ < 9485 and not jaDesceu then
                bv.Velocity = Vector3.new(0,0,0) -- TRAVA O BONECO NO AR
                ActionBtn.Text = "DESCER"
                ActionBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 0)
                ActionBtn.Visible = true
            
            -- Lógica 2: Chegou na areia final
            elseif posZ >= 9485 then
                bv.Velocity = Vector3.new(0,0,0) -- TRAVA O BONECO NA AREIA
                ActionBtn.Text = "PARAR DE VOAR"
                ActionBtn.BackgroundColor3 = Color3.fromRGB(150, 0, 0)
                ActionBtn.Visible = true
            
            -- Lógica 3: Voo normal (enquanto não estiver travado)
            else
...
ActionBtn.MouseButton1Click:Connect(function()
    ...
    if ActionBtn.Text == "DESCER" and h then
        h.CFrame = CFrame.new(-106, -17, h.Position.Z) -- Teleporta para baixo (espinhos)
        task.wait(0.2)
        jaDesceu = true -- Permite que o voo continue até a areia
    elseif ActionBtn.Text == "PARAR DE VOAR" then
        cleanup() -- Encerra o script para pegar o baú
    end
end)
